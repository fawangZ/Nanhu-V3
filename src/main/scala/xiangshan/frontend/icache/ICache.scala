  ...
  val dataArrays = (0 until partWayNum).map { w =>
    (0 until bank).map { b =>
      val sramBank = Module(new SRAMTemplate(
        ...
        hasMbist = coreParams.hasMbist,
        hasShareBus = coreParams.hasShareBus,
        parentName = parentName + s"bank${b}_way_${w}_"
      ))
      val mbistPipeline = Option.when(coreParams.hasMbist && coreParams.hasShareBus) {
        MBISTPipeline.PlaceMbistPipeline(1, s"${parentName}_mbistPipe${b}_${w}")
      }
